...
public class MessageControlador {
    ...
    public Message guardar(@RequestBody Message datos){
        return messageServicio.guardarMensaje(datos);
    }
    @GetMapping("/{numId}")
    public Optional<Message>getMessage(@PathVariable("numId")int id){
        return messageServicio.obtenerMensaje(id);
    }

    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Message update(@RequestBody Message datos){
        return messageServicio.update(datos);
    }
    @DeleteMapping("/{numId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public boolean eliminarMensaje(@PathVariable("numId")int id){
        return messageServicio.eliminarMensaje(id);
    }
